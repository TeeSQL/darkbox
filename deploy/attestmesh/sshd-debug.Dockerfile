...
#
# Public bastion ingress: the dstack gateway carries raw TCP only via TLS-passthrough
# (SNI-routed `<app_id>-<port>s.<domain>:443`), so when STUNNEL_ACCEPT_PORT is set the
# entrypoint fronts sshd with an stunnel TLS terminator on that port. Mesh-only
# sidecars leave it unset and serve plain sshd on :22 (reached over the wg mesh).
#
...
RUN apt-get update \
 && apt-get install -y --no-install-recommends \
      openssh-server curl jq ca-certificates iproute2 dnsutils netcat-openbsd stunnel4 openssl \
...
# 22 = inner sshd (reached over the mesh); 2222 = stunnel TLS-passthrough (public bastion only).
EXPOSE 22 2222
