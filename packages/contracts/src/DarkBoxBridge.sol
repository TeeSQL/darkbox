...
contract DarkBoxBridge is IDarkBoxBridge {
    ...
    function deposit(bytes32 gameId, uint256 amount, address beneficiary, bytes32 depositRef) external {
        ...
        uint256 balanceBefore = IERC20Minimal(usdc).balanceOf(address(this));
        _safeTransferFrom(usdc, msg.sender, address(this), amount);
        uint256 received = IERC20Minimal(usdc).balanceOf(address(this)) - balanceBefore;
        if (received != amount) revert ShortTokenReceipt(amount, received);

        totalDeposited[bene] += received;
        emit DepositReceived(gameId, msg.sender, bene, received, depositRef);
    }
