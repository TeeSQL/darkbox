...
contract ShortReceiptToken is MockERC20 {
    constructor() MockERC20("Short USDC", "sUSDC", 6) {}
...
contract DarkBoxBridgeTest is BridgeEIP712Helper {
    ...
    function test_ConstructorRejectsZeroSigner() public {
        ...
        new DarkBoxBridge(admin, address(0), address(usdc));
    }
    ...
    function test_DepositRejectsShortTokenReceipt() public {
        ShortReceiptToken shortUsdc = new ShortReceiptToken();
        DarkBoxBridge shortBridge = new DarkBoxBridge(admin, signer, address(shortUsdc));
        uint256 amount = 100e6;
        shortUsdc.mint(user, amount);

        vm.startPrank(user);
        shortUsdc.approve(address(shortBridge), amount);
        vm.expectRevert(abi.encodeWithSelector(DarkBoxBridge.ShortTokenReceipt.selector, amount, amount - 1));
        shortBridge.deposit(GAME_ID, amount, user, keccak256("short-receipt"));
        vm.stopPrank();
    }
    ...
    function _depositForUser(uint256 amount) internal {
        vm.startPrank(user);
        usdc.approve(address(bridge), amount);
        bridge.deposit(GAME_ID, amount, user, keccak256("test-deposit"));
        vm.stopPrank();
    }
