...
/** Public bridge ABI fragments used by the bridge service / smoke test (USDC-only). */
...
/** Shadow bridge controller ABI fragments (USDC-only: single asset, no asset arg). */
export const shadowBridgeControllerAbi = parseAbi([
  ...
  "function burnForWithdrawal(bytes32 withdrawalId, address owner, bytes32 shadowAccount, uint256 amount, bytes32 userCommandHash)",
  "function withdrawableBalance(bytes32 shadowAccount) view returns (uint256)",
  "function balanceOf(bytes32 shadowAccount) view returns (uint256)",
  "function shadowOf(address owner) view returns (bytes32)",
  "function setLocked(bytes32 shadowAccount, uint256 newLocked)",
...
/** ShadowMinted / ShadowBurned event items (name-addressed, index-independent). */
export const shadowMintedEvent = parseAbi([
  "event ShadowMinted(bytes32 indexed depositOpId, bytes32 indexed shadowAccount, uint256 amount)",
])[0];
export const shadowBurnedEvent = parseAbi([
  "event ShadowBurned(bytes32 indexed withdrawalId, bytes32 indexed shadowAccount, uint256 amount)",
])[0];
